...
在开始前首先介绍几个类，这几个类作用相似但是其实有一些不同，它们都用来表示和地址相关的信息。

**Address**

第一个类是`Address`类，这个类保有的变量有
  ...
  if (m_endPoint != 0)
    {
      ...
      // 是UdpSocketImpl类中的ForwardUp函数，调用者为自己，但由于NS3系统中实现了自己的智能指针，
      // 所以需要通过Ptr<UdpSocketImpl>进行包装后再传入。
